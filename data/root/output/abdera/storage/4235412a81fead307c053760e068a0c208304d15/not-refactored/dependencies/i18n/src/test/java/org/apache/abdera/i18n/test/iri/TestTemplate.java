...
public final class TestTemplate 
  ...
  public static void test13() throws Exception {
    ...
    HashMapContext c = new HashMapContext();
    c.put("term",new String("????r???????????????".getBytes("UTF-8"),"UTF-8"));  // not all java impl's are capable of handling this properly, so we have to force the conversion to UTF-8
    eval(t,e,c);
  }
  ...
  public static void test16() throws Exception {
    String t = "http://www.google.com/search?q={term}";
    String e = "http://www.google.com/search?q=????r???????????????";
    HashMapContext c = new HashMapContext();
    c.setIri(true);
    c.put("term",new String("????r???????????????".getBytes("UTF-8"),"UTF-8"));  // not all java impl's are capable of handling this properly, so we have to force the conversion to UTF-8
  ...
  public static void test17() throws Exception {
    String t = new String("bar{-prefix|/?/|var}/".getBytes("UTF-8"),"UTF-8");
    String e = new String("bar/?/foo/".getBytes("UTF-8"),"UTF-8");
    HashMapContext c = new HashMapContext();
    c.setIri(true);
    c.put("var","foo");
    eval(t,e,c);
